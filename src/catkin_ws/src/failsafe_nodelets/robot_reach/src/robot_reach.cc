...
RobotReach::RobotReach(std::vector<double> transformation_matrices, int nb_joints, std::vector<double> geom_par, ros::Publisher capsule_pub, bool advanced_verify = false,
            double x = 0, double y = 0, double z = 0, double roll = 0, double pitch = 0, double yaw = 0):
  nb_joints_(nb_joints),
  capsule_publisher_(capsule_pub),
  advanced_verify_(advanced_verify)
{
...
void RobotReach::reach(const custom_robot_msgs::StartGoalMotionConstPtr& potential_traj) {
  ...
  try{
    ...
    if (advanced_verify_) {
      custom_robot_msgs::StartGoalCapsuleArrayPtr robot_ra(new custom_robot_msgs::StartGoalCapsuleArray());
      ...
      capsule_publisher_.publish(robot_ra);
    } else {
      custom_robot_msgs::CapsuleArrayPtr robot_ra(new custom_robot_msgs::CapsuleArray());
      computeCapsulesFromStartGoalSegments(start_segments, goal_segments, s_diff, robot_ra->capsules);
      robot_ra->header.frame_id = potential_traj->header.frame_id;
      robot_ra->header.stamp = potential_traj->header.stamp;
      capsule_publisher_.publish(robot_ra);
