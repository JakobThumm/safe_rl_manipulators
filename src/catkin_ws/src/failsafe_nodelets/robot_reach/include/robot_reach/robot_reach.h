...
class RobotReach {
 private:
  ...
  /**
   * @brief robot's capsules publisher
   */
  ros::Publisher capsule_publisher_;

  /**
   * @brief If we use advanced verification, we additionally ouput the start and goal segment of each capsule.
   */
  bool advanced_verify_;
...
public:
   ...
   * @param capsule_pub robot's capsules publisher
   * @param advanced_verify use advanced verification or not
  ...
  RobotReach(std::vector<double> transformation_matrices, 
      ...
      ros::Publisher capsule_pub, 
      bool advanced_verify,
   ...
   * 
   * Computes and sends the reachable occupancy capsules of the robot to the ROS topic.
   */
  void reach(const custom_robot_msgs::StartGoalMotionConstPtr& potential_traj);
};
} // namespace robot_reach 
