...
FailsafePathConsistent::FailsafePathConsistent(int nb_joints, 
    double sample_time, 
    double t_buff, 
    double max_s_stop, 
    ...
    const ros::Publisher &ending_time_pub, 
    const ros::Publisher &start_goal_pub, 
    const ros::Publisher &motion_pub, 
    const ros::Publisher& cycle_finished_pub):
  ...
  ending_time_pub_(ending_time_pub),
  start_goal_pub_(start_goal_pub),
  motion_pub_(motion_pub),
  cycle_finished_pub_(cycle_finished_pub)
{
  sliding_window_k_ = (int) std::floor(max_s_stop_/sample_time_);
}


FailsafePathConsistent::~FailsafePathConsistent() {
  gazebo::client::shutdown();
};


bool FailsafePathConsistent::planFailsafePathConsistent(double pos, double vel, double acc, double ve, double a_max, double j_max, Path &path) {
  if (a_max < 0 || fabs(acc) > a_max) {
    ROS_DEBUG("planFailsafePathConsistent acc values incorrect. a_max = %f, acc = %f", a_max, acc);
    return false;
  }
  try {
    ...
    if (t01 < 0 || t12 < 0 || t23 < 0) {
      ROS_DEBUG("planFailsafePathConsistent calculated time negative. t01 = %f, t12 = %f, t23 = %f", t01, t12, t23);
      return false;
    }
    ...
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::planFailsafePathConsistent: " << exc.what());
    return false;
  }
}

void FailsafePathConsistent::calculateMaxAccJerk(const std::vector<double> &prev_speed, const std::vector<double>& a_max_part, const std::vector<double>& j_max_part, double& a_max_manoeuvre, double& j_max_manoeuvre) {
  double new_c, new_d;
...
void FailsafePathConsistent::computesPotentialTrajectory(bool v, const std::vector<double> &prev_speed)
{
  try {
    ...
    if (!recovery_path_.isCurrent()) {
      ...
      recovery_path_correct_ = planFailsafePathConsistent(failsafe_path_.getPosition(), failsafe_path_.getVelocity(), 
    ...
    if (recovery_path_correct_) {
      ...
      bool failsafe_2_planning_success = planFailsafePathConsistent(recovery_path_.getPosition(), recovery_path_.getVelocity(), recovery_path_.getAcceleration(), 0, a_max_manoeuvre, j_max_manoeuvre, failsafe_path_2_);
    ...
    // computes the potential trajectory

    // Publish the breaking time
    custom_robot_msgs::DoubleHeaderedPtr braking_time_msg(new custom_robot_msgs::DoubleHeadered());
    braking_time_msg->header.frame_id = "map";
    braking_time_msg->header.stamp = cycle_begin_time_;
    braking_time_msg->data = potential_path_.getPhase(3);
    ending_time_pub_.publish(braking_time_msg);
    ROS_DEBUG("Published t_brake");
    ...
    this->start_goal_pub_.publish(s_g_motion);
    //ROS_DEBUG("Published start goal motion");
  } catch (const std::exception &exc) {
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::computesPotentialTrajectory: " << exc.what());
  }
}


custom_robot_msgs::Motion FailsafePathConsistent::determineNextMotion(bool is_safe) {
...
void FailsafePathConsistent::publishMotion(const custom_robot_msgs::Motion& motion) {
  ...
  motion_pub_.publish(command_msg);
}


custom_robot_msgs::Motion FailsafePathConsistent::interpolateFromTrajectory(double s, double ds, double dds, const LongTermTraj& trajectory) const {
  try {
    ...
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::interpolateFromTrajectory: " << exc.what());
    return custom_robot_msgs::Motion();
  }

}

void FailsafePathConsistent::getPathSafety(const custom_robot_msgs::BoolHeaderedConstPtr& is_verified) {
  ROS_DEBUG_STREAM("FailsafePathConsistent::getPathSafety");
  try {
    is_safe_ = is_verified->data;

    // Increment the last safe path or the new path (iff verified) by one step
    next_motion_ = determineNextMotion(is_safe_);

    // Publish the motion
    publishMotion(next_motion_);

    // Publish that the cycle is finished
    std_msgs::Empty empty_msg;
    cycle_finished_pub_.publish(empty_msg);
  } catch (const std::exception &exc) {
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::getPathSafety: " << exc.what());
  }
}

void FailsafePathConsistent::getNextCycle(const custom_robot_msgs::BoolHeaderedConstPtr& next_cycle) {
  ROS_DEBUG("FailsafePathConsistent::getNextCycle, new_ltt = %d, new_ltt_processed = %d, is_safe = %d, new_goal = %d", new_ltt_, new_ltt_processed_, is_safe_, new_goal_);
  try {
    cycle_begin_time_ = next_cycle->header.stamp;
    ...
    // Compute a new potential trajectory and publish it.
    computesPotentialTrajectory(is_safe_, next_motion_.dq);
    new_ltt_processed_ = true;
  } catch (const std::exception &exc) {
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::getNextCycle: " << exc.what());
  }
}

void FailsafePathConsistent::initPotentialTrajectory(const std_msgs::EmptyConstPtr& data) {
  try {
    // starts the code
    ...
    computesPotentialTrajectory(false, prev_dq);
    //ROS_INFO("First potential trajectory published");
  } catch (const std::exception &exc) {
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::initPotentialTrajectory: " << exc.what());
  }
}

custom_robot_msgs::Motion FailsafePathConsistent::getCurrentMotion() {
...
bool FailsafePathConsistent::checkCurrentMotionForReplanning(const custom_robot_msgs::Motion& current_motion) {
  for (int i = 0; i < nb_joints_; i++) {
...
void FailsafePathConsistent::newLongTermTrajectory(const custom_robot_msgs::MotionConstPtr& goal_motion) {
  try { 
    ...
    ROS_ERROR_STREAM("Exception in FailsafePathConsistent::newLongTermTrajectory: " << exc.what());
  }
}


LongTermTraj FailsafePathConsistent::calculateLongTermTrajectory(const std::vector<double>& start_q, 
...
std::vector<double> FailsafePathConsistent::convertRMLVec(const RMLDoubleVector& rml_vec) {
